...
/**
 * <a href="https://leetcode.com/problems/word-search-ii/description/">https://leetcode.com/problems/word-search-ii/description/</a>
 */
public class WordSearchII {
    private static class TrieNode {
        ...
        public TrieNode getSubtree(Character character) {
            return this.characters.getOrDefault(character, null);
        }
    ...
    private void search(char[][] board, int i, int j, int depth, boolean[][] visited, TrieNode root, Set<Integer> availableWords) {
        if (i >= 0 && j >= 0 && i < board.length && j < board[0].length) {
            if (visited[i][j] || depth > 10) return;

            TrieNode subtreeRoot = root.getSubtree(board[i][j]);
            if (subtreeRoot == null) return;
            if (subtreeRoot.isWordEnd) availableWords.add(subtreeRoot.wordIndex);

            visited[i][j] = true;
            search(board, i + 1, j, depth + 1, visited, subtreeRoot, availableWords);
            search(board, i - 1, j, depth + 1, visited, subtreeRoot, availableWords);
            search(board, i, j + 1, depth + 1, visited, subtreeRoot, availableWords);
            search(board, i, j - 1, depth + 1, visited, subtreeRoot, availableWords);
            visited[i][j] = false;
        }
    }
    ...
    public List<String> findWords(char[][] board, String[] words) {
        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[0].length; j++) {
                ...
                search(board, i, j, 0, visited, root, availableWords);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(wsii.findWords(bigBoard, words));
    }
}
